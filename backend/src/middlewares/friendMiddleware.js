  ...
  try {
    ...
    if (recipientId) {
      ...
      if (blocked) {
        return res
          .status(403)
          .json({ message: "Không thể thao tác vì có quan hệ chặn giữa hai người dùng" });
      }
    ...
    const targetMemberIds =
      type === "direct" ? memberIds.slice(0, 1) : memberIds;

    const friendChecks = targetMemberIds.map(async (memberId) => {
      const blocked = await UserBlock.findOne({
        $or: [
          { blockerId: me, blockedId: memberId },
          { blockerId: memberId, blockedId: me },
        ],
      }).lean();

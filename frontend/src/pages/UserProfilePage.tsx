...
const STORY_MARKER_REGEX =
  /^\[\[story:(text|music|video)(?::([a-z0-9_-]+))?\]\]\s*/i;
const STORY_EXPIRE_MS = 24 * 60 * 60 * 1000;

const isStoryPost = (content?: string | null) =>
  STORY_MARKER_REGEX.test(content || "");

const stripStoryMarker = (content?: string | null) =>
  (content || "").replace(STORY_MARKER_REGEX, "").trim();
...
const UserProfilePage = () => {
  ...
  const [updatingContactVisibility, setUpdatingContactVisibility] =
    useState(false);
  ...
  const latestActiveStory = useMemo(() => {
    return (
      userPosts
        .filter(
          (post) => isStoryPost(post.content) && isStoryActive(post.createdAt),
        )
        .sort(
          (a, b) =>
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setIsMe(res.isMe);
        setIsFollowingProfile(Boolean(res.isFollowing));
        setFriendRequestSent(false);
        ...
        setStats({
          ...
          followerCount: res.followerCount ?? null,
          followingCount: res.followingCount ?? null,
  ...
  const currentContactVisibility =
    (currentUserInfo?.contactInfoVisibility as
      | "only_me"
      | "public"
      | "friends"
      | undefined) || "friends";
  ...
  useEffect(() => {
    ...
    isMe,
    currentUserInfo?._id,
    currentUserInfo?.hometown,
    currentUserInfo?.currentCity,
  ]);
  ...
  const handleUnfriend = async () => {
    ...
    try {
      ...
      setStats((prev) => ({
        ...prev,
        friendCount:
          typeof prev.friendCount === "number"
            ? Math.max(prev.friendCount - 1, 0)
            : prev.friendCount,
        mutualCount:
          typeof prev.mutualCount === "number" ? 0 : prev.mutualCount,
        previewAvatars: [],
      }));
  ...
  const handleCoverChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const nextCoverUrl = result?.coverUrl;
      if (!nextCoverUrl) {
        toast.error("Không nhận được URL ảnh bìa sau khi tải lên");
        return;
      }

      if (me) {
        setUser({ ...me, coverUrl: nextCoverUrl });
      }
      setProfileUser((prev) =>
        prev ? { ...prev, coverUrl: nextCoverUrl } : prev,
      );
      setCoverPreviewUrl((prev) => {
        if (prev) URL.revokeObjectURL(prev);
        return null;
      ...
      toast.error(error?.response?.data?.message || "Không thể tải ảnh bìa");
    } finally {
  ...
  const handleDeletePhoto = async () => {
    ...
    const targetPost = userPosts.find(
      (post) => post._id === photoViewerItem.postId,
    );
    if (!targetPost) {
    ...
    const keepMediaUrls = media
      ...
      .map((item) => item.url);
    const isPostEmptyAfterDelete =
      keepMediaUrls.length === 0 && !(targetPost.content || "").trim();
    if (isPostEmptyAfterDelete) {
      toast.warning(
        "Không thể xoá ảnh cuối cùng của bài viết không có nội dung.",
      );
      return;
    }
  ...
  const handleSaveBio = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Không thể cập nhật tiểu sử",
      );
    } finally {
  ...
  const handleChangeContactVisibility = async (
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Không thể cập nhật quyền xem",
      );
    } finally {
  ...
  const handleSaveRequiredLocation = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Không thể cập nhật thông tin nơi ở",
      );
    } finally {
  ...
  const handleToggleFollowVerified = () => {
    const targetUserId = currentUserInfo?._id;
    ...
    const run = async () => {
      try {
        ...
        setIsFollowingProfile(Boolean(result?.isFollowing));
        setStats((oldStats) => ({
          ...oldStats,
          followerCount:
            typeof result?.followerCount === "number"
              ? result.followerCount
              : oldStats.followerCount,
          followingCount:
            typeof result?.followingCount === "number"
              ? result.followingCount
              : oldStats.followingCount,
        }));
      } catch (error: any) {
        toast.error(
          error?.response?.data?.message || "Không thể cập nhật theo dõi",
        );
      }
    };
  ...
  return (
    <main className="hc-page-bg min-h-screen p-3 sm:p-4">
      <div className="hc-profile-shell mx-auto max-w-5xl space-y-3">
        <Button
          variant="outline"
          size="sm"
          className="border-slate-300/90 bg-white/90 hover:bg-white"
          onClick={() => navigate("/")}
        >
        ...
        <Card className="hc-profile-hero p-0">
            ...
            <div
              ...
              style={{
                backgroundImage: coverPreviewUrl
                  ? `url(${coverPreviewUrl})`
                  : currentUserInfo?.coverUrl
                    ? `url(${currentUserInfo.coverUrl})`
              ...
              <>
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                    ...
                    <div>
                      <h1 className="flex flex-wrap items-center gap-2 text-2xl sm:text-3xl font-bold leading-tight">
                        {currentUserInfo.displayName}
                        {currentUserInfo.isVerified ? (
                          <VerifiedBadge className="h-[0.82em] w-[0.82em] translate-y-[2px]" />
                        ) : null}
                      </h1>
                      ...
                      <p className="mt-2 text-base sm:text-lg font-medium">
                        {currentUserInfo.isVerified ? (
                          <span>{stats.followerCount ?? 0} người theo dõi</span>
                        ) : (
                          <button
                            type="button"
                        ...
                        {currentUserInfo.isVerified
                          ? `${stats.followingCount ?? 0} đang theo dõi`
                      ...
                      <div className="mt-2 flex -space-x-2">
                        {stats.previewAvatars.length > 0
                          ? stats.previewAvatars.map((avatarUrl, index) => (
                              <Avatar
                                key={`${avatarUrl}-${index}`}
                                className="h-8 w-8 border-2 border-background sm:h-9 sm:w-9"
                              >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {isMe ? (
                      ...
                      <Button
                        variant={isFollowingProfile ? "secondary" : "outline"}
                        className="h-10 px-4 text-sm"
                        onClick={handleToggleFollowVerified}
                      >
                    ...
                    {!isMe && currentUserInfo?.isVerified && (
                ...
                {isMe && currentUserInfo && (
                  <Dialog
                    open={editingProfile}
                    onOpenChange={setEditingProfile}
                  >
                    <DialogContent className="w-[calc(100vw-24px)] max-w-3xl p-0 sm:rounded-2xl">
                      ...
                      <div className="max-h-[80vh] overflow-y-auto p-4 sm:p-5">
                        {showMissingInfoPrompt && (
                          <div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 p-3 text-amber-900">
                            <button
                              type="button"
                              className="float-right rounded-full p-1 text-amber-700 hover:bg-amber-100"
                              onClick={() => setShowMissingInfoPrompt(false)}
                              title="Đóng"
                            >
                              <X className="size-4" />
                            </button>
                            <p className="pr-8 text-sm font-medium">
                              Bạn chưa cập nhật "Sống ở" hoặc "Quê quán". Vui
                              lòng nhập để hoàn thiện hồ sơ.
                            </p>
                          </div>
                        )}
                        <ProfilePersonalInfoInlineForm
                ...
                {activeSection === "friends" ? (
                  <Card className="hc-elevated-card">
                    <CardHeader>
                  ...
                  <div className="grid gap-4 lg:grid-cols-3">
                    <Card className="hc-elevated-card lg:col-span-1">
                      <CardHeader>
                      ...
                      <CardContent className="space-y-3 text-sm">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-foreground">
                            {formatDateVi(currentUserInfo.birthday)}
                          </p>
                        </div>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-foreground">
                              {currentUserInfo.relationshipStatus ===
                                "in_relationship" &&
                        ...
                        <div className="space-y-3 border-t pt-3">
                            ...
                            {isMe && (
                              <DropdownMenu>
                                ...
                                >
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleChangeContactVisibility("only_me")
                                    }
                                  >
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleChangeContactVisibility("public")
                                    }
                                  >
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleChangeContactVisibility("friends")
                                    }
                                  >
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex items-start gap-3">
                              ...
                              <p className="text-foreground">
                                Điện thoại:{" "}
                                {currentUserInfo.phone || "Chưa cập nhật"}
                              </p>
                            </div>
                            <div className="flex items-start gap-3">
                              ...
                              <p className="text-foreground">
                                Email:{" "}
                                {currentUserInfo.email || "Chưa cập nhật"}
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    <Card className="hc-elevated-card lg:col-span-2">
                      <CardHeader>
                                ...
                                {post.sharedPost && (
                                  <div className="mx-4 mb-3 overflow-hidden rounded-xl border">
                                    <div className="px-3 py-2">
                                      <div className="flex items-center gap-2">
                                        <Avatar className="h-8 w-8 border border-border/60">
                                          ...
                                          />
                                          <AvatarFallback>
                                            {post.sharedPost.author.displayName?.charAt(
                                              0,
                                            ) || "U"}
                                          </AvatarFallback>
                                        </Avatar>
                                    ...
                                    {post.sharedPost.isUnavailable ? (
                                      ...
                                      (post.sharedPost.media || []).length >
                                        0 && (
                                        <div className="space-y-1 border-t">
                                          {(post.sharedPost.media || []).map(
                                            (item, index) => (
                                              <div
                                ...
                                <div className="border-t px-2 py-1">
                                  <div className="grid grid-cols-3 gap-1">
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        toast.info(
                                          "Bình luận sẽ được cập nhật tiếp",
                                        )
                                      }
                                    >
            ...
            <div className="space-y-3 p-4">
              {latestActiveStory.media?.[0]?.url ? (
                ...
                <div className="rounded-xl bg-muted p-4 text-center text-sm text-muted-foreground">
                  {stripStoryMarker(latestActiveStory.content) ||
                    "Story văn bản"}
                </div>
              )}
              <p className="text-xs text-muted-foreground">
                {formatPostAgo(latestActiveStory.createdAt)}
              </p>
            </div>
          ) : (
            <div className="p-4 text-sm text-muted-foreground">
              Hiện chưa có story nào còn hiệu lực.
            </div>
          )}
        </DialogContent>
      </Dialog>
      ...
      >
        ...
        >
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Bạn cần nhập{" "}
              <span className="font-semibold text-foreground">quê quán</span> để
              tiếp tục dùng trang cá nhân. Mục{" "}
